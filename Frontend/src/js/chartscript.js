...
function PerformanceRating(data) {
    return data.map(item => {
        const fields = Object.keys(item)[0].split(';');
        const values = Object.values(item)[0].split(';');
        const index = fields.indexOf('LeavesTaken');
...
function SelfMotivationLevel(data) {
    return data.map(item => {
        const fields = Object.keys(item)[0].split(';');
        const values = Object.values(item)[0].split(';');
        const index = fields.indexOf('LeavesTaken');
...
function fetchAndDisplayData() {
    fetch(`http://127.0.0.1:5000/employees?id=${id}`)  // Adjust the URL as needed
        ...
        .then(data => {
            ...
            const performanceRating = data[0].PerformanceRating;
            const selfMotivationLevel = data[0].SelfMotivationLevel;
            ...
            updateChart('performanceRatingChart', 'Performance Rating', performanceRating, 'rgba(255, 99, 132, 0.2)', 'rgba(255, 99, 132, 1)');
            updateChart('selfMotivationLevelChart', 'Self Motivation Level', selfMotivationLevel, 'rgba(54, 162, 235, 0.2)', 'rgba(54, 162, 235, 1)');
        })
